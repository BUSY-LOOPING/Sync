...
//        long milli = getDuration(musicFiles.get(holder.getAdapterPosition()).getPath());
    ...
    public static class MiniPlayerRecyclerViewAdapterViewHolder extends RecyclerView.ViewHolder {
    ...
    public void update(ArrayList<MusicFiles> musicFiles) {
        this.musicFiles = new ArrayList<>(musicFiles);
        notifyDataSetChanged();
    }

    public void updateNowPlaying(int pos) {
        nowPlaying = musicFiles.get(pos);
        if (pos != -1) {
